  ...
  do {
    try {
      ...
      const resp = await fetch(
        "https://toncenter.com/api/v3/accountStates?" + params,
        {
          headers,
        },
      );
...
describe("Config custom slot", () => {
    ...
    voteOut = async (propHash, critical, codeUpgrade) => {
      ...
      do {
        ...
        for (let i = 0; i < curVset.total; i++) {
          ...
          if (
            !(codeUpgrade && minWins - winCount == 1 && weightRemaining <= 0)
          ) {
            const voteTx = findTransactionRequired(res.transactions, {
              on: deployer.address,
              from: configAddress,
              op: (op) =>
                op == Op.voteProcessed + 2 ||
                op == Op.voteProcessed + 6 + Number(critical), // Processed successfully
            });
            if (
              voteTx.inMessage!.body.beginParse().preloadUint(32) ==
              Op.voteProcessed + 6 + Number(critical)
            ) {
              winCount++;
            }
          } else {
          ...
          if (propAccepted) {
            ...
            const newVset = packValidatorsSet({
              ...curVset,
              utime_since: blockchain.now + 100,
              utime_unitl: blockchain.now + 100 + 65536,
            });
            const res = await blockchain.sendMessage(
              internal({
                from: electorAddress,
  ...
  it("should be able to set custom slot -1024", async () => {
    const testSlot = -1024;
    const testCellA = beginCell()
      .storeStringTail("Hop hey La La Ley")
      .endCell();
    const testCellB = beginCell().storeBuffer(testCellA.hash()).endCell();
    for (let testCell of [testCellA, testCellB]) {
      const setSlotMsg = Config.setCustomSlotMessage(
        testSlot,
        testCell,
        deployer.address,
      );
      ...
      expect(
        configSlotBefore === undefined || !configSlotBefore.equals(testCell),
      ).toBe(true);
  ...
  it("only admin should be able to set slots", async () => {
    let testSenders = [
      deployer.address,
      randomAddress(0),
      new Address(-1, customSlotAdmin.hash),
      differentAddress(customSlotAdmin),
    ];
    ...
    for (let testSlot of customSlots) {
      for (let testAddr of testSenders) {
        ...
        const res = await configContract.sendSetCustomSlot(
          blockchain.sender(testAddr),
          testSlot,
          testCell,
          deployer.address,
        );
  ...
  it("should bounce set_custom_slot when response address is not standard", async () => {
    ...
    customSlots.forEach((slot) => {
      const varAddressPayload = beginCell()
        ...
        .endCell();
      testPayloads.push(
        ...testCases.map((a) => Config.setCustomSlotMessage(slot, testCell, a)),
        varAddressPayload,
      );
    });
  ...
  it("should not allow more than single cell for custom slot", async () => {
    ...
    const testCell = beginCell()
      .storeStringRefTail("Hop hey La La Ley")
      .endCell();

    for (let testSlot of customSlots) {
      const dataBefore = await configContract.getData();
      const res = await configContract.sendSetCustomSlot(
        blockchain.sender(customSlotAdmin),
        testSlot,
        testCell,
        deployer.address,
      );
  ...
  it("should not accept exotic cells for custom slot", async () => {
    ...
    let testProofPrep = beginCell()
      .storeUint(3, 8) // Merkle proof
      .storeBuffer(testCell.hash()) // Hash
      .storeUint(testCell.depth(), 16)
      .storeRef(testCell)
      .endCell();
    const testProof = new Cell({
      exotic: true,
      bits: testProofPrep.bits,
      refs: testProofPrep.refs,
    });

    const testUpdatePrep = beginCell()
      .storeUint(4, 8) // Merkle update
      ...
      .storeRef(testProof)
      .endCell();
    const testUpdate = new Cell({
      exotic: true,
      bits: testUpdatePrep.bits,
      refs: testUpdatePrep.refs,
    });
  ...
  it("should not allow to create proposals for parameters -1024 and -1025", async () => {
    ...
    const critParams = Dictionary.loadDirect(
    ...
    const testCreateVoting = async (testParams: number[], expectOp: number) => {
      for (let paramId of testParams) {
        ...
        expect(
          await configContract.getProposal(propMsg.refs[0].hash()),
        ).toBeNull();
  ...
  it("check that votes work", async () => {
    ...
    const codeParam = (await configContract.getConfig()).get(-1000);
    const codeParamHash = codeParam ? codeParam.hash() : Buffer.alloc(32);
  ...
  it("test that proposal for parameters -1024 -1025 can't be accepted", async () => {
    ...
    const origCell = beginCell()
      .storeStringTail("Hop hey La La Ley!")
      .endCell();
    const currentConfig = await configContract.getConfig();
    const currentCodeParam = currentConfig.get(-1000);
    const currentCodeParamHash = currentCodeParam
      ? currentCodeParam.hash()
      : Buffer.alloc(32);
    ...
    const codeProposalMsg = Config.newVotingProposalMessage({
      ...
      cur_hash: currentCodeParamHash,
    });

    const slotProposals = customSlots.map((s) =>
      Config.newVotingProposalMessage({
        expire_at: Math.floor(Date.now() / 1000) + 3600 * 24 * 60,
        critical: false,
        param_id: s,
        value: testCell,
        cur_hash: currentConfig.get(s)?.hash() ?? Buffer.alloc(32),
      }),
    );
    ...
    for (let updateSlot of customSlots) {
      await configContract.sendSetCustomSlot(
        blockchain.sender(customSlotAdmin),
        updateSlot,
        origCell,
        deployer.address,
